...
router.put("/:cid", async (req, res) => {
    ...
    try {
        ...
        if (uid !== channel.owner && channel.type !== 0) {
            return resCode(res, 401, "Not authorized");
        }
        channel.name = name;
        await channel.save();
        resJSON(res, "success", 200, "Channel updated");
    } catch (e) {
        console.error(e);
        resCode(res, 500);
    }
...
router.get("/:cid/member", async (req, res) => {
    ...
    try {
        ...
        const members = await User.find({uid: {$in: membersIds.map(member => member.uid)}}).select("display_name username avatar settings.status.status");
